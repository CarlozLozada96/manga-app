...
class MangaController < ApplicationController
  ...
  def chapter_pages
    ...
    chapters_list_response = Rails.cache.fetch("chapters_list_#{@manga_id}", expires_in: 5.minutes) do
      @manga_service.get_chapters(@manga_id)['data']
    end

    @chapters_list = fetch_sorted_chapters(chapters_list_response)
    @chapter_number = fetch_chapter_number(@chapter_id, chapters_list_response)
    current_chapter_index = @chapters_list.find_index { |c| c['id'] == @chapter_id }
    @current_chapter = @chapters_list[current_chapter_index]
  ...
  def previous_chapter_id(current_chapter_index)
    current_chapter_index > 0 ? @chapters_list[current_chapter_index - 1]['id'] : nil
  end

  def next_chapter_id(current_chapter_index)
    current_chapter_index < @chapters_list.length - 1 ? @chapters_list[current_chapter_index + 1]['id'] : nil
  end
